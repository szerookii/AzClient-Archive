...
void Utils::DebugLogOutput(std::string str) {
	WriteToFile(str, "AzClient/logs.txt");
	return;
}
...
bool Utils::isKeyDown(int key) {
	...
	if (keyMapOffset == 0x0) {
		...
		if (sigOffset != 0x0) {
			...
			keyMapOffset = sigOffset - getModuleBase() + offset + 7;
		}
	}

	return *reinterpret_cast<bool*>(getModuleBase() + keyMapOffset + ((uintptr_t)key * 0x4));
}